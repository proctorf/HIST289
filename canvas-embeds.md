# Canvas LMS Embed Codes for HIST 289 Schedule

Use these iframe embed codes in Canvas to link directly to specific weeks in the syllabus.

**Base URL**: `https://proctorf.github.io/HIST289/`
*(Replace with your actual GitHub Pages URL or hosting URL)*

## Week 1 - Course Introduction
```html
# Canvas LMS Integration - HIST 289 Weekly Content

This document provides iframe embed codes for each week's content to be inserted into Canvas LMS modules.

## Base URL Setup
Your GitHub Pages site: `https://proctorf.github.io/HIST289/`

## Week 1 - Course Introduction
```html
<iframe src="https://proctorf.github.io/HIST289/week1" 
        width="100%" 
        height="600" 
        frameborder="0" 
        scrolling="auto"
        title="Week 1 - Course Introduction">
</iframe>>
```

## Week 2 - Digital Humanities and Slavery
```html
<iframe src="https://proctorf.github.io/HIST289/week2" 
        width="100%" 
        height="800" 
        frameborder="0" 
        scrolling="auto"
        title="Week 2 - Digital Humanities and Slavery">
</iframe>
```

## Week 3 - QGIS and Data
```html
<iframe src="https://proctorf.github.io/HIST289/week3" 
        width="100%" 
        height="700" 
        frameborder="0" 
        scrolling="auto"
        title="Week 3 - QGIS and Data">
</iframe>
```

## Week 4 - QGIS and Ohio Slavery
```html
<iframe src="https://proctorf.github.io/HIST289/week4" 
        width="100%" 
        height="700" 
        frameborder="0" 
        scrolling="auto"
        title="Week 4 - QGIS and Ohio Slavery">
</iframe>
```

## Week 5 - Website Assessment and Database Construction
```html
<iframe src="https://proctorf.github.io/HIST289/week5" 
        width="100%" 
        height="500" 
        frameborder="0" 
        scrolling="auto"
        title="Week 5 - Website Assessment and Database Construction">
</iframe>
```

## Week 6 - Runaway Slaves and QGIS
```html
<iframe src="https://proctorf.github.io/HIST289/week6" 
        width="100%" 
        height="700" 
        frameborder="0" 
        scrolling="auto"
        title="Week 6 - Runaway Slaves and QGIS">
</iframe>
```

## Week 7 - Runaway Slaves and StoryMaps
```html
<iframe src="https://proctorf.github.io/HIST289/week7" 
        width="100%" 
        height="800" 
        frameborder="0" 
        scrolling="auto"
        title="Week 7 - Runaway Slaves and StoryMaps">
</iframe>
```

## Week 8 - XML and Text Encoding
```html
<iframe src="https://proctorf.github.io/HIST289/week8" 
        width="100%" 
        height="700" 
        frameborder="0" 
        scrolling="auto"
        title="Week 8 - XML and Text Encoding">
</iframe>
```

## Week 9 - XML and Website Assessment
```html
<iframe src="https://proctorf.github.io/HIST289/week9" 
        width="100%" 
        height="500" 
        frameborder="0" 
        scrolling="auto"
        title="Week 9 - XML and Website Assessment">
</iframe>
```

## Week 10 - Slave Narratives and Distant Reading
```html
<iframe src="https://proctorf.github.io/HIST289/week10" 
        width="100%" 
        height="800" 
        frameborder="0" 
        scrolling="auto"
        title="Week 10 - Slave Narratives and Distant Reading">
</iframe>
```

## Week 11 - Distant Reading and Digital Humanities
```html
<iframe src="https://proctorf.github.io/HIST289/week11" 
        width="100%" 
        height="700" 
        frameborder="0" 
        scrolling="auto"
        title="Week 11 - Distant Reading and Digital Humanities">
</iframe>
```

## Week 12 - Data Visualization
```html
<iframe src="https://proctorf.github.io/HIST289/week12" 
        width="100%" 
        height="800" 
        frameborder="0" 
        scrolling="auto"
        title="Week 12 - Data Visualization">
</iframe>
```

## Week 13 - Digital Tools and Final Projects
```html
<iframe src="https://proctorf.github.io/HIST289/week13" 
        width="100%" 
        height="500" 
        frameborder="0" 
        scrolling="auto"
        title="Week 13 - Digital Tools and Final Projects">
</iframe>
```

## Week 14 - Final Project Work
```html
<iframe src="https://proctorf.github.io/HIST289/week14" 
        width="100%" 
        height="500" 
        frameborder="0" 
        scrolling="auto"
        title="Week 14 - Final Project Work">
</iframe>
```

## Week 15 - Final Presentations
```html
<iframe src="https://proctorf.github.io/HIST289/week15" 
        width="100%" 
        height="600" 
        frameborder="0" 
        scrolling="auto"
        title="Week 15 - Final Presentations">
</iframe>
```

## Instructions for Canvas Implementation

1. **Copy the iframe code** for the week you want to embed
2. **In Canvas**, navigate to your course and go to Modules
3. **Add a new item** to the appropriate module
4. **Select "External URL"** as the item type
5. **Paste the iframe code** into the URL field, or use the HTML editor
6. **Adjust height values** as needed based on content length
7. **Save and publish** the module item

## Alternative Direct Links

If iframes don't work in your Canvas setup, you can use direct links:

- Week 1: https://proctorf.github.io/HIST289/week1
- Week 2: https://proctorf.github.io/HIST289/week2
- Week 3: https://proctorf.github.io/HIST289/week3
- Week 4: https://proctorf.github.io/HIST289/week4
- Week 5: https://proctorf.github.io/HIST289/week5
- Week 6: https://proctorf.github.io/HIST289/week6
- Week 7: https://proctorf.github.io/HIST289/week7
- Week 8: https://proctorf.github.io/HIST289/week8
- Week 9: https://proctorf.github.io/HIST289/week9
- Week 10: https://proctorf.github.io/HIST289/week10
- Week 11: https://proctorf.github.io/HIST289/week11
- Week 12: https://proctorf.github.io/HIST289/week12
- Week 13: https://proctorf.github.io/HIST289/week13
- Week 14: https://proctorf.github.io/HIST289/week14
- Week 15: https://proctorf.github.io/HIST289/week15

## Notes
- Heights are estimated based on content length and may need adjustment
- All links assume your site is deployed to GitHub Pages at the standard URL
- Test the iframes in Canvas preview mode before publishing to students
- Consider Canvas accessibility requirements when implementing iframes
```

## Week 2 - Digital Humanities & Slavery
```html
<iframe src="https://proctorf.github.io/HIST289/week2" width="100%" height="600" frameborder="0"></iframe>
```

## Week 3 - QGIS & Data
```html
<iframe src="https://proctorf.github.io/HIST289/#week3" width="100%" height="600" frameborder="0"></iframe>
```

## Week 4 - QGIS & Ohio Slavery
```html
<iframe src="https://proctorf.github.io/HIST289/#week4" width="100%" height="600" frameborder="0"></iframe>
```

## Week 5 - Website Assessment & Database Construction
```html
<iframe src="https://proctorf.github.io/HIST289/#week5" width="100%" height="600" frameborder="0"></iframe>
```

## Week 6 - Runaway Slaves & QGIS
```html
<iframe src="https://proctorf.github.io/HIST289/#week6" width="100%" height="600" frameborder="0"></iframe>
```

## Week 7 - Runaway Slaves & Digital History Articles
```html
<iframe src="https://proctorf.github.io/HIST289/#week7" width="100%" height="600" frameborder="0"></iframe>
```

## Week 8 - XML Encoding
```html
<iframe src="https://proctorf.github.io/HIST289/#week8" width="100%" height="600" frameborder="0"></iframe>
```

## Week 9 - XML Tutorial & Assessment
```html
<iframe src="https://proctorf.github.io/HIST289/#week9" width="100%" height="600" frameborder="0"></iframe>
```

## Week 10 - Slave Narratives & Distant Reading
```html
<iframe src="https://proctorf.github.io/HIST289/#week10" width="100%" height="600" frameborder="0"></iframe>
```

## Week 11 - Distant Reading & Digital Humanities Data
```html
<iframe src="https://proctorf.github.io/HIST289/#week11" width="100%" height="600" frameborder="0"></iframe>
```

## Week 12 - Visualizations & Data Humanities
```html
<iframe src="https://proctorf.github.io/HIST289/#week12" width="100%" height="600" frameborder="0"></iframe>
```

## Week 13 - Digital Tools & Final Projects
```html
<iframe src="https://proctorf.github.io/HIST289/#week13" width="100%" height="600" frameborder="0"></iframe>
```

## Thanksgiving Break
```html
<iframe src="https://proctorf.github.io/HIST289/#thanksgiving" width="100%" height="600" frameborder="0"></iframe>
```

## Week 14 - Final Project Work
```html
<iframe src="https://proctorf.github.io/HIST289/#week14" width="100%" height="600" frameborder="0"></iframe>
```

## Week 15 - Final Presentations
```html
<iframe src="https://proctorf.github.io/HIST289/#week15" width="100%" height="600" frameborder="0"></iframe>
```

---

## Alternative: Direct Links (for Canvas Pages)

If you prefer to use direct links instead of iframes, use these URLs:

- **Week 1**: https://proctorf.github.io/HIST289/week1
- **Week 2**: https://proctorf.github.io/HIST289/week2
- **Week 3**: https://proctorf.github.io/HIST289/#week3
- **Week 4**: https://proctorf.github.io/HIST289/#week4
- **Week 5**: https://proctorf.github.io/HIST289/#week5
- **Week 6**: https://proctorf.github.io/HIST289/#week6
- **Week 7**: https://proctorf.github.io/HIST289/#week7
- **Week 8**: https://proctorf.github.io/HIST289/#week8
- **Week 9**: https://proctorf.github.io/HIST289/#week9
- **Week 10**: https://proctorf.github.io/HIST289/#week10
- **Week 11**: https://proctorf.github.io/HIST289/#week11
- **Week 12**: https://proctorf.github.io/HIST289/#week12
- **Week 13**: https://proctorf.github.io/HIST289/#week13
- **Thanksgiving Break**: https://proctorf.github.io/HIST289/#thanksgiving
- **Week 14**: https://proctorf.github.io/HIST289/#week14
- **Week 15**: https://proctorf.github.io/HIST289/#week15

---

## Canvas Module Setup Instructions

1. **Enable GitHub Pages** for your HIST289 repository
2. **Update the base URL** in the embed codes above with your actual GitHub Pages URL
3. **Create Canvas modules** for each week
4. **Add Pages** to each module using the iframe embed codes
5. **Alternative**: Use the direct links as external URLs in Canvas modules

## Embed Code Customization

You can adjust the iframe parameters:
- `width="100%"` - Makes the iframe responsive
- `height="600"` - Adjust height as needed (try 400-800px)
- `frameborder="0"` - Removes iframe border
- Add `scrolling="yes"` if you want to ensure scrolling is enabled
